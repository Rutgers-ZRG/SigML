...
if not(read_dataset):
    dataset = build_data(patoms, pfps, sig_texts=None, efs=pefs, device=device, radial_cutoff=5.0)
...
# leg_lmax = dataset[0].sig.shape[2]
...
# full_sig_model = get_standard_full_sig_model(ave_neighbors, leg_lmax, cutoff=5.0, device=device)
# opt = torch.optim.AdamW(full_sig_model.parameters(), lr=0.01, weight_decay=0.01)
# scheduler = torch.optim.lr_scheduler.StepLR(opt, step_size=10, gamma=0.5)
# loss_fn = torch.nn.SmoothL1Loss(reduction="sum")
# # loss_fn = torch.nn.L1Loss(reduction="sum")
# save_path = "SAVED_MODELS/full_sig_model_all.pth"
# 
# # full_sig_model.load_state_dict(torch.load(save_path))
# #train_full_sig(full_sig_model, opt, train_data, loss_fn, scheduler, save_path = save_path, max_iter=3, val_percent = 0.1, device=device, batch_size=1)
# full_sig_model.load_state_dict(torch.load(save_path))
# for o in range(5):
#     evaluate_full_sig_legendre(full_sig_model, test_data[3:], orbital=o, atom=1, N=2, display=True)
# exit()
############################################################
...
### Custom E3NN Model ###
ef_model = get_standard_ef_model(ave_neighbors, mul=32, cutoff=5.0, device=device)
opt = torch.optim.AdamW(ef_model.parameters(), lr=0.01, weight_decay=0.01)
scheduler = torch.optim.lr_scheduler.StepLR(opt, step_size=5, gamma=0.5)
loss_fn = torch.nn.SmoothL1Loss(reduction="sum")
save_path = "SAVED_MODELS/ef_model.pth"

# ef_model.load_state_dict(torch.load(save_path))
train_ef(ef_model, opt, train_data, loss_fn, scheduler, save_path = save_path, max_iter=20, val_percent = 0.1, device=device, batch_size=1)
