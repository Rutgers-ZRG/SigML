...
def get_spacegroup_atom(atom):
    spa = SPA(AAA.get_structure(atom), symprec=1e-5)
    return spa.get_space_group_number()
...
def build_datapoint(atom, type_encoding, type_onehot, am_onehot, fps, sig_text=None, ef=None, r_max=3.0, device="cpu"):
    ...
    if sig_text is not None:
        ...
        sig = torch.as_tensor(sig, dtype=torch.complex64, device=device).unsqueeze(0)
    ...
    x = torch.as_tensor(fps, dtype=torch.float32, device=device)
    z = type_onehot[[type_encoding[s] for s in symbols]].to(device)
    ...
    data = tg.data.Data(pos = positions, lattice = lattice, symbol = symbols, 
                        ...
                        cor_atom_inds = cor_atom_inds, eq_inds = eq_inds)
    return data
...
def train_full_sig(model, optimizer, dataset, loss_fn, scheduler, save_path = None, max_iter=101, val_percent = 0.1, device="cpu", batch_size=1):
    model.to(device)
    train_data, val_data = train_test_split(dataset, train_percent=(1 - val_percent), seed=53234)
    train_loader = DataLoader(train_data,batch_size=batch_size,shuffle=True,num_workers=0, collate_fn=collate_to_list)
    val_loader = DataLoader(val_data, batch_size=batch_size, shuffle=False,num_workers=0, collate_fn=collate_to_list)
    for step in range(max_iter):
        model.train()
        loss_cumulative = 0.0
        for batch in tqdm(train_loader, desc=f"Epoch {step+1}", leave=False):
            outputs = model(batch)
            targets = [b.sig.squeeze(0) for b in batch]
            loss = sum(loss_fn(o, t.to(device)) for o, t in zip(outputs, targets))
            loss_cumulative += loss.detach().cpu().item()
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        model.eval()
        ...
        with torch.no_grad():
            for vbatch in val_loader:
                val_outs = model(vbatch)
                val_targs = [b.sig.squeeze(0) for b in vbatch]
                loss = sum(loss_fn(o, t.to(device)) for o, t in zip(val_outs, val_targs))
                loss_val += loss.detach().cpu().item()
        train_loss_avg = loss_cumulative / (len(train_loader)*batch_size)
        val_loss_avg   = loss_val / (len(val_loader)*batch_size)
        print("Epoch", str(step+1),"Train Loss =", train_loss_avg,"Val Loss =", val_loss_avg)
...
def evaluate_full_sig(model, dataset_org, orbital, display=True, img_save_dir = None):
    dataset = copy.deepcopy(dataset_org)
    model.eval()
    ...
    for i in range(N1):
        for j in range(N2):
            ...
            if i == 0 and j ==0:
                axs[i,j].plot(iws[0], dataset[N2*i + j].sig[0,0, :, orbital, 0].detach().cpu().numpy(), c="black", label="True DMFT $\Sigma$(i$\omega_n$)")
                axs[i,j].plot(iws[0], output[0, :, orbital, 0], c="red", marker='o', markersize=2, label="Predicted DMFT Re{$\Sigma$(i$\omega_n$)}")
                axs[i,j].plot(iws[0], dataset[N2*i + j].sig[0,0, :, orbital, 1].detach().cpu().numpy(), c="black")
                axs[i,j].plot(iws[0], output[0, :, orbital, 1], c="blue", marker='o', markersize=2, label="Predicted DMFT Im{$\Sigma$(i$\omega_n$)}")
            else:
                axs[i,j].plot(iws[0], dataset[N2*i + j].sig[0,0, :, orbital, 0].detach().cpu().numpy(), c="black")
                axs[i,j].plot(iws[0], output[0, :, orbital, 0], c="red", marker='o', markersize=2)
                axs[i,j].plot(iws[0], dataset[N2*i + j].sig[0,0, :, orbital, 1].detach().cpu().numpy().real, c="black")
...
def train_ef(model, optimizer, dataset, loss_fn, scheduler, save_path = None, max_iter=101, val_percent = 0.1, device="cpu", batch_size=1):
    ...
    train_data, val_data = train_test_split(dataset, train_percent= 1-val_percent, seed=44123)
    train_loader = DataLoader(train_data, batch_size=batch_size,shuffle=True,num_workers=0, collate_fn=collate_to_list)
    val_loader = DataLoader(val_data, batch_size=batch_size, shuffle=False,num_workers=0, collate_fn=collate_to_list)
    ...
    for step in range(max_iter):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {step+1}", leave=False):
            outputs = model(batch)
            targets = torch.stack([b.ef[0] for b in batch], dim=0).squeeze(-1)
            # if batch_size == 1:
            #     output = output.unsqueeze(0)
        ...
        # val_dataloader = copy.deepcopy(val_data)
        loss_val = 0
        for vi, v in enumerate(val_loader):
            val_out = model(v)
            val_targets = torch.stack([b.ef[0] for b in v], dim=0).squeeze(-1)
            # if batch_size == 1:
            #     val_out = val_out.unsqueeze(0)
            # if vi == 0:
            #     print(val_out)
            #     print(v.ef)
            # print(val_out, v.ef[0])
            loss = loss_fn(val_out, val_targets)
            loss_val += loss.cpu().detach().item()

        print("Epoch", str(step+1),"Train Loss =", loss_cumulative/len(train_loader), "Val Loss =", loss_val/len(val_loader))
        # scheduler.step(loss_val)
        scheduler.step()
    if save_path is not None:
        print("Saving model to", save_path)
...
def evaluate_ef(model, dataset_org, device="cpu"):
    dataset = copy.deepcopy(dataset_org)
    model.to(device)
    model.eval()
    ...
    for i in range(len(dataset)):
        dataset[i].to(device)
        preds.append(model([dataset[i]]).squeeze(-1).cpu().detach().item()/len(dataset[i].symbol))
        acts.append(dataset[i].ef[0].item()/len(dataset[i].symbol))
...
def build_data(atoms, sig_texts=None, efs=None, fps=None, radial_cutoff=3.0, device="cpu"):
    type_encoding = {}
    species_am = []
    ...
    for i in tqdm(range(len(atoms)), desc="Building dataset..."):
        ...
        if efs is not None:
            tef = efs[i]
        if passed:
            adata = build_datapoint(atoms[i], type_encoding, type_onehot, am_onehot, fps[i], tsig_text, tef, radial_cutoff, device=device)
