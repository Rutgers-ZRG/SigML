...
class PeriodicNetworkReal(Network):
    ...
    def forward(self, data_inp: Union[tg.data.Data, Dict[str, torch.Tensor]]) -> torch.Tensor:
        ...
        #     if i not in data.cor_atom_inds:
        #         output[i] = 0.0


        ### Zero out the uncorrelated atoms and set self energies of equivalent atoms to be the same ###
        uncor_atom = torch.arange(output.shape[0], dtype=torch.long, device=output.device)
        uncor_atom = uncor_atom[~torch.isin(uncor_atom, data.cor_atom_inds)]
        output[uncor_atom] = 0.0

        for i in range(len(data.eq_inds)):
            for j in data.eq_inds[i]:
                output[data.eq_inds[i][0]] = output[j]
