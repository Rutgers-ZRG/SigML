...
def build_datapoint(atom, type_encoding, type_onehot, fps, sig_text=None, ef=None, r_max=4.0, leg_lmax=30, device="cpu"):
    ...
    ### Set to 118 to match network settings for now ###
    x = torch.tensor(fps[:,:118]).to(torch.float32).to(device)
...
def get_average_neighbor_count(all_data):
    ...
    """
    neighbor_count = []
    for adata in tqdm(all_data, desc="Getting average neighbor count..."):
        N = adata.pos.shape[0]
        for i in range(N):
...
def evaluate_sinf(model, dataset_org, display=True, img_save_dir = None, device="cpu"):
    ...
    for i in range(len(dataset)):
        ...
        maes += np.sum(np.abs(preds[i] - acts[i]), axis=0)
        mae_count += len(preds[i])
    print("MAE: ", ' & '.join([f"{maes[i]/mae_count:.3f}" for i in range(n_orbitals)]))
...
def evaluate_ef(model, dataset_org):
    ...
    for i in range(len(dataset)):
        ...
        acts.append(act)
    mae /= len(dataset)
    plt.scatter(preds, acts, c="black")
    ...
    plt.text(np.amin(acts)+0.5, np.amax(acts)-0.5, f"MAE: {mae:.5f} eV", fontsize=15)

    plt.xlabel("ML Predicted E$_f$ (eV)", fontsize=15)
    plt.ylabel("True E$_f$ (eV)", fontsize=15)
    plt.show()
...
def build_data(atoms, fps, leg_lmax=30, sig_texts=None, efs=None, radial_cutoff=3.0, device="cpu"):
    r"""
    ...
    ----------
      ...
      fps: list of np.ndarray
        Fingerprint matrices for each structure
      leg_lmax: int, default = 30
        The maximum legendre order considered when calculating the legendre coefficients from self-energy data
    ...
    for i in tqdm(range(len(atoms)), desc="Building dataset..."):
        ...
        if efs is not None:
            tef = efs[i]
        if passed:
            adata = build_datapoint(atoms[i], type_encoding, type_onehot, fps[i], tsig_text, tef, radial_cutoff, leg_lmax, device=device)
